...
contains
    ...
    subroutine parse_input(schema, filename, tree, ounit)
        ...
        call build_logical_lines(lines, sources, line_nums, llines, &
    ...
    subroutine build_logical_lines(lines, sources, line_nums, llines, &
        ...
        do while (i <= n)
            ...
            do while (ends_with_backslash(combined))
                ...
                i = i + 1
                if (i > n) then
                    call stop_error(ounit, start_source, start_line, &
                        'line continuation missing')
                endif
    ...
    subroutine parse_lines(schema, llines, lsources, lnums, tree, ounit)
        ...
        do i = 1, size(llines)
            ...
            if (.not. in_block) then
                if (normalize_name(line) == 'END') then
                    call stop_error(ounit, lsources(i), lnums(i), &
                ...
                if (iblock_def == 0) then
                    call stop_error(ounit, lsources(i), lnums(i), &
                        'unknown block: ' // trim(line))
                endif
                if (find_block_data_index(tree, bname) > 0) then
                    call stop_error(ounit, lsources(i), lnums(i), &
                        'duplicate block: ' // trim(line))
                endif
                in_block = .true.
                ...
                if (normalize_name(line) == 'END') then
                    ...
                    if (find_block_index(schema, normalize_name(line)) > 0) then
                        call stop_error(ounit, lsources(i), lnums(i), &
                            'block not closed before new block')
                    endif
                    ...
                    if (find_key_index(schema%blocks(iblock_def), kname) == 0) &
                        then
                        call stop_error(ounit, lsources(i), lnums(i), &
                            'unknown key: ' // trim(token))
                    endif
                    if (find_pair_index(current, kname) > 0) then
                        call stop_error(ounit, lsources(i), lnums(i), &
                            'duplicate key: ' // trim(token))
                    endif
                    call append_pair(current, kname, rest, &
                        lsources(i), lnums(i))
                endif
            endif
        enddo
        ...
        if (in_block) then
            call stop_error(ounit, current%source, current%start_line, &
                'block not closed: ' // trim(current%name))
        endif
        ...
        do i = 1, size(schema%blocks)
            if (schema%blocks(i)%required) then
                if (find_block_data_index(tree, schema%blocks(i)%name) == 0) &
                    then
                    write(ounit,*) 'error: required block missing: ', &
                        trim(schema%blocks(i)%name)
                    stop
                endif
            endif
        enddo
