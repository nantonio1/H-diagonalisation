module fparse_io
    use fparse_util, only: strip_comment, is_blank, split_first_token, &
        normalize_name, is_absolute_path, get_dirname, join_path
    implicit none
...
contains
    ...
    subroutine load_file_lines(filename, lines, sources, line_nums, ounit)
        ...
        depth = 0
        call load_file_recursive(filename, lines, sources, line_nums, &
            stack, depth, ounit)
